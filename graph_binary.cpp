...
// Reads in a binary file to create a binary graph
...
Graph::Graph(unsigned int n, unsigned long m, double t, vector<unsigned long> d, vector<unsigned int> l, vector<float> w) {
  nb_nodes     = n;
  nb_links     = m;
  total_weight = t;
  degrees      = d;
...
// Writes binary graph into outfile
