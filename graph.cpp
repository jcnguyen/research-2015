...
Graph::Graph(char *filename, int type) {
  ...
  while (!finput.eof()) {
    ...
    if (finput) {
      ...
      // add link to an existing vertex
      links[src].push_back(make_pair(dest,weight));

      // reverse the src and dest
      if (src!=dest)
...
void Graph::display(int type) {
  for (unsigned int src=0 ; src<links.size(); src++) {
    for (unsigned int j=0 ; j<links[src].size(); j++) {
      int dest   = links[src][j].first;
      float weight = links[src][j].second;
      if (type==WEIGHTED)
