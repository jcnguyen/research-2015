...
//                mergeCommunities() function here (see comments in that 
//                function), and an indexing adjustment in printHeapTop10() 
//                in maxheap.h.
//
///////////////////////////////////////////////////////////////////////////////
...
void buildDeltaQMatrix() {
	...
	// binary search trees, each of which represents the dQ_{x,j} adjacency
	// vector. Having created dQ as such, we may happily delete the matrix e
	// in order to free up more memory. 
	// The next step is to create a max-heap data structure, which contains
	// the entries of the following form (value, s, t), where the heap-key is 
	// 'value'. Accessing the root of the heap gives us the next dQ value, and
	// the indices (s,t) of the vectors in dQ which need to be updated as a
	// result of the merge.
	
	
	// First we compute e_{i,j}, and the compute+store the a_{i} values. 
	...
	// While computing each dQ for a row i, we track the maximum dQmax of the
	// row and its (row,col) indices (i,j). Upon finishing all dQ's for a row,
	...
	for (int i=1; i<gparm.maxid; i++) {
		if (e[i].so != 0) {
			...
			while (current->next != NULL) {        
				...
				dq[i].v->insertItem(current->si, dQ); // insert into vector[i]
			}
...
void mergeCommunities(int i, int j) {
	...
	// update the dQ values which correspond to any neighbor of either i or j 
	// to reflect the change. In doing this, there are three update rules 
	// (cases) to follow:
	...
	// For the first two cases, we may make these updates by simply getting a 
	// list of the elements (x,dQ) of [i] and stepping through them. If x==j, 
	// then we can ignore that value since it corresponds to an edge which is 
	// being absorbed by the joined community (i,j). If [j] also contains an 
	// element (x,dQ), then we have a triangle; if it does not, then we have a 
	// jix-chain.
	// The last case requires that we step through the elements (x,dQ) of [j] 
	// and update each if [i] does not have an element (x,dQ), since that implies a ijx-chain. 
	// Let d([i]) be the degree of the vector [i], and let k = d([i]) + d([j]).
	// The running time of this operation is O(k log k)
	...
	// Now that we have a list of the elements of [i], we can step through 
	// them to check if they correspond to an jix-triangle, a jix-chain, or 
	// the edge (i,j), and do the appropriate operation depending.
	...
	while (current!=NULL) { // insert list elements appropriately
		...
		// If the element (x,dQ) is actually (j,dQ), then we can ignore it, 
		// since it will correspond to an edge internal to the joined 
		// community (i,j) after the join.
		if (current->x != j) {

			// Now we must decide if we have a jix-triangle or a jix-chain by 
			// asking if [j] contains some element (x,dQ). If the following 
			// conditional is TRUE, then we have a jix-triangle, ELSE it is a 
			// jix-chain.
			
			if (dq[j].v->findItem(current->x)) { // CASE OF JIX-TRIANGLE
				...
				// We first add (x,dQ) from [i] to [x] as (j,dQ), 
				// since [x] essentially now has two connections to the 
				// joined community [j].
				if (ioparm.textFlag>1) {
				...
				// Then we need to delete the element (i,dQ) in [x], 
				// since [i] is now a part of [j] and [x] must reflect this 
				// connectivity.
				if (ioparm.textFlag>1) { 
				...
				// maximum element. If it's new, then
...
// 				Change suggested by Janne Aukia (jaukia@cc.hut.fi) on 12 Oct 2006
	...
	// We've now finished going through all of [i]'s connections, 
	// so we need to delete the element of [j] which represented the 
	// connection to [i]
	if (ioparm.textFlag>1) {
		cout << endl;
	...
	while (current != NULL) { // insert list elements appropriately
		if (ioparm.textFlag>1) { 
			cout << endl << "element["<<t<<"] from dq["<<j<<"] is ("<<current->x<<" "<<current->y<<")" << endl; 
		}

		// If the element (x,dQ) of [j] is not also (i,dQ) (which it shouldn't 
		// be since we've already deleted it previously in this function), 
		// and [i] does not also have an element (x,dQ), 
	...
	// Now that we've updated the connections values for all of [i]'s and [j]
	// 's neighbors, we need to update the a[] vector to reflect the change in
	// fractions of edges after the join operation.
	if (ioparm.textFlag>1) {
		cout << endl;
...
bool parseCommandLine(int argc,char * argv[]) {
	...
	while (argct < argc) {
		...
		if (temp == "-files") {
			...
			for (int i=0; i < ioparm.filename.size(); i++) { 
